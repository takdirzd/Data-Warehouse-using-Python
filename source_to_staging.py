...
def import_excel_to_stg(cursor = None,
                        DB_SCHEMA = None,
                        PATH_TO_EXCEL = None):
    # Baca file Exce
    data_excel = pd.read_excel(f'{PATH_TO_EXCEL}')
    ...
    for row in data_excel.itertuples():
        sql = f"""INSERT INTO {DB_SCHEMA}.Superstore (
                    Row_ID,
                    Order_ID,
                    Order_Date,
                    Ship_Date,
...
def import_json_to_stg(cursor = None,
                        DB_SCHEMA = None,
                        PATH_TO_JSON = None):
    # Baca file JSON
    with open(f'{PATH_TO_JSON}') as f:
    ...
    for item in data_json:
        sql = f"INSERT INTO {DB_SCHEMA}.people (person, region) VALUES (%s, %s)"
        cursor.execute(sql, (item['Person'], item['Region']))  # Sesuaikan dengan nama kolom
...
def import_xml_to_stg(cursor = None,
                        DB_SCHEMA = None,
                        PATH_TO_XML = None):

    # Parsing file XML
    file_path = f'{PATH_TO_XML}'
    ...
    for index, row in df_new.iterrows():
        sql = f"INSERT INTO {DB_SCHEMA}.returns (Order_ID, Returned) VALUES (%s, %s)"
        cursor.execute(sql, (row['Order_ID'], row['Returned']))
