...
if load_dotenv() == False:
    ...
    if connection_stg[1] == True:
        ...
        import_excel_to_stg(prm_CURSOR = cursor_stg, prm_DB_SCHEMA= DB_SCHEMA_STG, prm_PATH_TO_EXCEL = PATH_TO_EXCEL)
        import_json_to_stg(prm_CURSOR = cursor_stg, prm_DB_SCHEMA= DB_SCHEMA_STG, prm_PATH_TO_JSON = PATH_TO_JSON)
        import_xml_to_stg(prm_CURSOR = cursor_stg, prm_DB_SCHEMA= DB_SCHEMA_STG, prm_PATH_TO_XML = PATH_TO_XML)
        conn_stg.commit()
        ...
        # STAGING TO DWH
        region_mgr(prm_CURSOR_STG = cursor_stg,
                    prm_CURSOR_DWH = cursor_dwh,
                    prm_DB_SCHEMA_STG = DB_SCHEMA_STG,
                    prm_DB_SCHEMA_DWH = DB_SCHEMA_DWH
                    )
        location(prm_CURSOR_STG = cursor_stg,
                    prm_CURSOR_DWH = cursor_dwh,
                    prm_DB_SCHEMA_STG = DB_SCHEMA_STG,
                    prm_DB_SCHEMA_DWH = DB_SCHEMA_DWH
                    )

        product(prm_CURSOR_STG = cursor_stg,
                    prm_CURSOR_DWH = cursor_dwh,
                    prm_DB_SCHEMA_STG = DB_SCHEMA_STG,
                    prm_DB_SCHEMA_DWH = DB_SCHEMA_DWH
                    )
        
        customer(prm_CURSOR_STG = cursor_stg,
                    prm_CURSOR_DWH = cursor_dwh,
                    prm_DB_SCHEMA_STG = DB_SCHEMA_STG,
                    prm_DB_SCHEMA_DWH = DB_SCHEMA_DWH
                    )
        
        orders(prm_CURSOR_STG = cursor_stg,
                    prm_CURSOR_DWH = cursor_dwh,
                    prm_DB_SCHEMA_STG = DB_SCHEMA_STG,
                    prm_DB_SCHEMA_DWH = DB_SCHEMA_DWH
                    )
